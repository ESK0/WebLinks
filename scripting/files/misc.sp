...
{    
    strcopy(sFetchMethod, sizeof(sFetchMethod), "steamid");

    if (!bCSGO)
        return;

    KeyValues kvConfig = new KeyValues("WebLinks");
    ...
    if (kvConfig.JumpToKey("Settings")) {
        ...
        kvConfig.GetString("FetchMethod", sFetchMethod, sizeof(sFetchMethod), "steamid");
        kvConfig.GetString("FallbackUrl", sFallbackUrl, sizeof(sFallbackUrl), "");
    }
...
public void UpdateMotdFile()
{
    ...
    File motdFile = OpenFile("motd.txt", "w+");

    if (motdFile == null)
		return;

    char sFileContent[1024];

    Format(sFileContent, sizeof(sFileContent), "%s/method/%s?fallbackUrl=%s", WEBLINKS_API, sFetchMethod, sFallbackUrl);
    motdFile.WriteLine(sFileContent);
